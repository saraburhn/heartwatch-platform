...
def simulate():
    ...
    # get last few readings to help status detection
    ...
    if mode == "normal":
        ...
        # random
        bpm = random.choices(
            population=[random.randint(60, 90), random.randint(121, 150), random.randint(35, 44), random.randint(155, 190)],
...
def alert():
    ...
    location = request.form.get("location", "GPS: 29.3759, 47.9774 (demo)")  # demo coordinates
    contacts = db.execute("SELECT name, phone, email FROM contacts WHERE user_id=?", (uid,)).fetchall()
    recipients = ", ".join(
...
def history():
    ...
    uid = session["user_id"]
    readings = db.execute(
        "SELECT ts, bpm, status, label FROM readings WHERE user_id=? ORDER BY ts DESC LIMIT 200", (uid,)
    ).fetchall()
    alerts = db.execute(
        "SELECT ts, bpm, location, recipients FROM alerts WHERE user_id=? ORDER BY ts DESC LIMIT 50", (uid,)
    ).fetchall()
